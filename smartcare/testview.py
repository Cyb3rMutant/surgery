...
class TestViews(TestCase):
    ...
    def test_home_view_authenticated_patient(self):
        user = User.objects.create_user(
            username="test_patient", password="password", role="patient"
        )
    ...
    def test_patient_signup_view(self):
        response = self.client.get(reverse("patient_signup"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "registration/signup.html")

    def test_doctor_signup_view(self):
        response = self.client.get(reverse("doctor_signup"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "registration/signup.html")
    ...
    def test_make_appointment_view_post(self):
        ...
        response = self.client.post(
            reverse("make-appointment"),
            {"description": "Sample appointment description", "date": "5/5/2025"},
        )
    ...
    def test_appoint_view(self):
        user = User.objects.create_user(
            username="test_doctor", password="password", role="doctor"
        )
    ...
    def test_view_prescriptions_view(self):
        ...
        self.client.login(username="test_patient", password="password")
        appointment = Appointment.objects.create(
            patient=user, description="Test Appointment", date=timezone.now().date()
        )
        prescription = Prescription.objects.create(
            appointment=appointment, doctor=user, details="Test Prescription", price=100
        )
    ...
    def test_payments_per_month_view(self):
        ...
        response = self.client.get(reverse("view-payments"))
